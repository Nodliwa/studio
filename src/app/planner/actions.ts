 ...
 * Requires SMTP_HOST, SMTP_PORT, SMTP_USER, and SMTP_PASS environment variables.
 */
export async function sendCollaboratorInvite(data: {
  ...
  // Validate configuration before attempting to create a transporter
  if (!SMTP_HOST || !SMTP_PORT || !SMTP_USER || !SMTP_PASS) {
    console.error('--- SMTP CONFIGURATION MISSING ---');
    console.log(`Invite for ${data.collaboratorName} (${data.collaboratorEmail})`);
    console.log(`Link: ${data.planUrl}`);
    console.log('---------------------------------');
    return { 
      success: false, 
  ...
  try {
    ...
    return { 
      success: false, 
      message: 'Failed to send email. Please check your SMTP credentials and connection.' 
    };
  }
}
