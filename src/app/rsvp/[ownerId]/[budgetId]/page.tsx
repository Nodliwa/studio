...
export default function PublicRsvpPage({ params: { ownerId, budgetId } }: { params: { ownerId: string, budgetId: string } }) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-secondary p-4">
        <Card className="max-w-md w-full text-center">
          <CardHeader>
            ...
            <CardDescription>The link you followed may be incorrect or the celebration is no longer active.</CardDescription>
          </CardHeader>
          <CardFooter>
            <Button className="w-full" onClick={() => window.location.href = '/'}>Go to SimpliPlan</Button>
          </CardFooter>
        </Card>
      </div>
    );
  ...
  const eventDate = budget?.eventDate ? new Date(budget.eventDate).toLocaleDateString("en-ZA", {
    year: "numeric",
    month: "long",
    day: "numeric",
  }) : 'TBD';
